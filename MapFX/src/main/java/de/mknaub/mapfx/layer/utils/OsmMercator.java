...
public class OsmMercator {
    ...
    public static double radius(int aZoomlevel) {
        return (TILE_SIZE * (1 << aZoomlevel)) / (2.0 * PI);
    }
    ...
    public static int latToY(double aLat, int aZoomlevel) {
        ...
        double sinLat = sin(toRadians(aLat));
        double log = log((1.0 + sinLat) / (1.0 - sinLat));
        int mp = getMaxPixels(aZoomlevel);
        int y = (int) (mp * (0.5 - (log / (4.0 * PI))));
        y = min(y, mp - 1);
        return y;
    }
    ...
    public static double YToLat(int aY, int aZoomlevel) {
        ...
        double latitude = (PI / 2) - (2 * atan(exp(-1.0 * aY / radius(aZoomlevel))));
